...
export declare type SelectablePropertyKeys<T extends Selectable> = T extends TObject<infer U>
  ? keyof U
  : T extends TRef<TObject<infer U>>
  ? keyof U
  : never
export declare type SelectableProperties<T extends Selectable> = T extends TObject<infer U>
  ? U
  : T extends TRef<TObject<infer U>>
  ? U
  : never
export declare type UnionToIntersect<U> = (U extends any ? (k: U) => void : never) extends (k: infer I) => void
  ? I
  : never
...
export declare type StaticRequiredPropertyKeys<T extends TProperties> = keyof Omit<
  T,
  StaticReadonlyOptionalPropertyKeys<T> | StaticReadonlyPropertyKeys<T> | StaticOptionalPropertyKeys<T>
>
export declare type StaticIntersectEvaluate<T extends readonly TSchema[]> = {
  [K in keyof T]: T[K] extends TSchema ? Static<T[K]> : never
}
export declare type StaticIntersectReduce<I extends unknown, T extends readonly any[]> = T extends [infer A, ...infer B]
  ? StaticIntersectReduce<I & A, B>
  : I
...
export declare type StaticRecord<K extends TRecordKey, T extends TSchema> = K extends TString
  ...
  : K extends TNumber
  ? Record<number, Static<T>>
  : K extends TKeyOf<TKey[]>
  ? Record<K['$static'], Static<T>>
  : K extends TUnion<TSchema[]>
  ? Record<K['$static'], Static<T>>
  : never
...
export declare type StaticIntersect<T extends readonly TSchema[]> = StaticIntersectReduce<
  unknown,
  StaticIntersectEvaluate<T>
>
...
export declare type StaticConstructor<T extends readonly TSchema[], U extends TSchema> = new (
  ...args: [...StaticParameters<T>]
) => Static<U>
export declare type StaticFunction<T extends readonly TSchema[], U extends TSchema> = (
  ...args: [...StaticParameters<T>]
) => Static<U>
...
export declare class TypeBuilder {
  ...
  KeyOf<T extends TObject<TProperties> | TRef<TObject<TProperties>>>(
    object: T,
    options?: CustomOptions,
  ): TKeyOf<SelectablePropertyKeys<T>[]>
  /** `Standard` Makes all properties in the given object type required */
  Required<T extends TObject<TProperties> | TRef<TObject<TProperties>>>(
    object: T,
    options?: ObjectOptions,
  ): TObject<StaticRequired<T['properties']>>
  /** `Standard` Makes all properties in the given object type optional */
  Partial<T extends TObject<TProperties> | TRef<TObject<TProperties>>>(
    object: T,
    options?: ObjectOptions,
  ): TObject<StaticPartial<T['properties']>>
  /** `Standard` Picks property keys from the given object type */
  Pick<T extends TObject<TProperties> | TRef<TObject<TProperties>>, K extends SelectablePropertyKeys<T>[]>(
    object: T,
    keys: [...K],
    options?: ObjectOptions,
  ): TObject<Pick<SelectableProperties<T>, K[number]>>
  /** `Standard` Omits property keys from the given object type */
  Omit<T extends TObject<TProperties> | TRef<TObject<TProperties>>, K extends SelectablePropertyKeys<T>[]>(
    object: T,
    keys: [...K],
    options?: ObjectOptions,
  ): TObject<Omit<SelectableProperties<T>, K[number]>>
